...
async function createGitlabMergeRequestWithFile(
  options: MergeRequestOptions,
): Promise<MergeRequest> {
  try {
    const response = await fetch('/api/infraweave-backend-gitlab/pull-request-with-file', {
      method: 'POST',
      headers: {
...
export default createGitlabMergeRequestWithFile;
